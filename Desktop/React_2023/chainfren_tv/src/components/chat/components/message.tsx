...
import { FaBan, FaFlag, FaVolumeMute, FaVolumeOff, FaExclamationTriangle, FaUserTimes, FaEllipsisH } from "react-icons/fa";
  ...
  return (
    <div className="flex flex-col">
      <div
        key={timetoken}
        className={clsx(
          "flex flex-col rounded-lg px-3 py-2 border border-pubnub-white text-white group/item"
        )}
      >
            ...
            <div className="flex flex-row">
              {isBroadcaster && flagCount > 0 && (
                <div className="flex flex-row items-center mr-2">
                  ...
                  <FaFlag className="text-pubnub-yellow ml-1" />
                </div>
              )}
              {!bannedUser?.ban && (
                <button
                ...
                >
                  {bannedUser?.mute ? (
                    <FaVolumeMute className="h-4 w-4 text-pubnub-light-grey" />
                  ) : (
                    <FaVolumeOff className="h-4 w-4 text-pubnub-white" />
                  )}
                </button>
              )}
              <button
            ...
            <div className="flex flex-row items-center">
              {isMessageFlagged && (
                <div className="p-2 justify-center">
                    <FaExclamationTriangle className={clsx("h-4 w-4 text-pubnub-yellow")} />
                </div>
              )}
              {isUserFlagged && (
                <div className="p-2 justify-center">
                    <FaUserTimes className={clsx("h-4 w-4 text-pubnub-red")} />
                </div>
              )}
              {!isMessageFlagged && !isUserFlagged && (
                ...
                >
                  <FaEllipsisH className="h-4 w-4 text-pubnub-white" />
                </button>
              )}
            </div>
          )}
